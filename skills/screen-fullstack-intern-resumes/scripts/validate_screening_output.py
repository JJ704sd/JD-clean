#!/usr/bin/env python3
"""Validate a full-stack intern resume-screening record and human-review gate."""
...
BASIC_CRITERIA = {"INT-WEB-01", "INT-FE-01", "INT-DATA-01"}
...
def _validate_recommendation(
    ...
    if recommendation == "advance_pending_human":
        ...
        basic_supported = sum(
            ...
            and STRENGTH_RANK.get(evidence.get(cid, {}).get("strength"), -1) >= STRENGTH_RANK["E1"]
            for cid in BASIC_CRITERIA
        )
        ...
        decision_criteria = set(ADVANCE_MINIMUMS) | {
            ...
            if evidence.get(criterion, {}).get("state") == "supported"
        }
        if any(
        ...
        ai = evidence.get("INT-AI-01", {})
        if ai.get("state") != "supported" and "INT-AI-01" not in probe_criteria:
            errors.append("missing AI evidence on an advance record requires an INT-AI-01 interview probe")
        ...
        admin_directly_not_met = any(
            evidence.get(cid, {}).get("state") == "directly_not_met"
            for cid in ("INT-ADM-01", "INT-AVAIL-01")
        )
        no_programming_project = all(
            evidence.get(cid, {}).get("state") == "not_evidenced"
            and evidence.get(cid, {}).get("strength") in {"E0", "E1"}
            for cid in ("INT-BE-01", "INT-PROJECT-01")
        )
        fragmented_implementation_evidence = any(
            evidence.get(cid, {}).get("state") == "supported"
            and STRENGTH_RANK.get(evidence.get(cid, {}).get("strength"), -1) >= 2
            for cid in BASIC_CRITERIA | {"INT-QUALITY-01"}
        )
        if not (
            admin_directly_not_met
            or (no_programming_project and not fragmented_implementation_evidence)
        ):
            errors.append("do_not_advance_pending_human lacks the intern negative evidence gate")
        if any(
            evidence.get(criterion, {}).get("confidence") == "low"
            and evidence.get(criterion, {}).get("state") in {"not_evidenced", "directly_not_met"}
            for criterion in {"INT-ADM-01", "INT-AVAIL-01", "INT-BE-01", "INT-PROJECT-01"}
        ):
            errors.append("low-confidence negative gate requires second review")
        if (
            no_programming_project
            and fragmented_implementation_evidence
            and not admin_directly_not_met
        ):
            errors.append("fragmented implementation evidence requires boundary review before a negative recommendation")
        summary = record.get("recruiter_summary")
        if isinstance(summary, dict) and not summary.get("critical_gaps"):
...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        return 1
    print(f"valid full-stack intern screening record (schema {record['schema_version']})")
    return 0
