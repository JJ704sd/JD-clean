...
description: "筛选跨境物流高级全栈工程师的单份或批量简历，以 Go/Node.js 项目证据为主栈门槛并突出 Go、重构和物流经验，生成可追溯的非最终建议。用于简历初筛；不用于实习生、面试评分或自动招聘决定。"
---
...
3. 需要判断主栈边界、优先信号、边界负面结论或其他模糊案例时，再读取[校准案例](references/decision-examples.md)。
...
- Go 或 Node.js 是后端主栈硬门槛，必须有至少 `E2` 的项目级个人交付证据。只有 Java、Python、PHP 或其他非目标后端栈时不满足门槛，不因相邻栈深度转迁移能力二审；仅在技能列表出现 Go/Node.js 的 `E1` 关键词也不满足。
- 主栈分层为：有合格 Go 证据（无论是否同时具备 Node.js）优先；只有合格 Node.js 证据可保留，但优先级较低。具体 Go/Node.js 框架名不是硬卡项。
...
3. 按后端项目证据填写 `priority_profile.target_stack`：`go_present`、`nodejs_only`、`no_qualifying_go_or_nodejs` 或 `unclear`。再分别填写重构和物流经验的 `supported`、`not_evidenced` 或 `unclear`；每个 `supported` 必须能回指对应 criterion 的项目证据。
4. 对每项标记 `supported`、`not_evidenced`、`conflicting` 或 `directly_not_met`，并按 `E0`–`E3` 记录证据强度、原文位置、理由和证据归类置信度。`conflicting` 只表示简历来源事实互相矛盾，必须使用 `U03_CONFLICTING_FACTS`；`directly_not_met` 只用于候选人有可定位的明确陈述与已批准条件相反，不能由“未写”或关键词缺失推断。
...
9. 用户要求结构化审计记录时，再附加符合 `schema_version: 1.2`、`rubric_version: senior-fullstack-2026-08-25-v5` 的 JSON。不得替人填写审核人、审核决定或盲审完成确认。

## 批量输入

- 先用同一 JD/rubric 版本独立完成每份记录，再汇总审核队列。
- 汇总可以按三个建议状态分组，并显示主栈层级与两个优先信号；同等核心门槛下人工选人时先看 Go，再在同一主栈层级内看重构和物流经验。不得用学校、公司品牌或无关批次背景排名。
