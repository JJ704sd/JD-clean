...
资深全栈 v10 按四项条件累计判断：学历、物流经验、高含金量项目、语言转换与学习证据。达到两项不符合才建议暂不推进；只有一项不符合仍保留推进资格。没有物流经验时，学历满足且有高含金量项目与语言适配/转语言学习交付证据的候选人可以推进。高含金量项目重点核对业务量、使用量或覆盖、个人参与程度和业务复杂度；WMS、CRM、VMS/TMS、ERP 等只作为检索线索，不能脱离项目上下文自动判定。薪资、地点、到岗、沟通取消和岗位意愿继续与能力判断分离。
...
索引默认只展示当前岗位合同下 `score >= 70` 且 Python 生成的 `review_status=advance_pending_human` 的候选人；二审、人工复核或低于展示阈值者不会进入列表。v10 的建议状态先由四项条件计数生成：0–1 项不符合为待人工推进，2–4 项不符合为待人工暂不推进；评分只表示证据覆盖。阈值可用 `--screening-min-score 80` 或环境变量 `FEISHU_SCREENING_MIN_SCORE` 调整；它不是最终录用决定。
...
证据强度固定换算为 `E0=0%`、`E1=40%`、`E2=75%`、`E3=100%`。资深全栈 v10 中，模型只返回事实清单，Python 生成证据等级和四项条件状态；E3 必须同时具备项目背景、个人动作、方法或取舍、结果口径和可核验影响，缺一项最多 E2。
...
证据档位不覆盖岗位规则：学历、物流经验、高含金量项目、语言转换与学习证据分别映射为 `met`、`not_met` 或 `unclear`，并由 `unmet_requirement_count` 应用两项阈值。Go 项目或已完成的转语言/转栈学习交付都可满足语言项；自评态度不能。旧版 v2–v9 记录继续只读兼容，新任务使用 v10 重筛并保留版本信息。
