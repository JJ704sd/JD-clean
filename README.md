...
资深全栈 v11 只按学历、物流经验、高含金量项目三项累计判断：达到两项不符合才建议暂不推进，只有一项不符合仍保留推进资格。语言匹配、转语言经历和学习态度因简历阶段难以稳定取证，仅作非阻断参考与面试追问。高含金量项目重点核对业务量、使用量或覆盖、个人参与程度和业务复杂度；WMS、CRM、VMS/TMS、ERP 等只作为检索线索，不能脱离项目上下文自动判定。
...
索引默认只展示当前岗位合同下 `score >= 70` 且 Python 生成的 `review_status=advance_pending_human` 的候选人；二审、人工复核或低于展示阈值者不会进入列表。v11 的建议状态由三项主条件计数生成：0–1 项不符合为待人工推进，2–3 项不符合为待人工暂不推进；语言信号和评分均不替代该规则。
...
证据强度固定换算为 `E0=0%`、`E1=40%`、`E2=75%`、`E3=100%`。资深全栈 v11 中，模型只返回事实清单，Python 生成证据等级、三项主条件状态和独立语言参考信号。
...
证据档位不覆盖岗位规则：学历、物流经验、高含金量项目分别映射为 `met`、`not_met` 或 `unclear`，并由 `unmet_requirement_count` 应用两项阈值。语言与学习单独记录，不计入该数量。旧版 v2–v10 记录继续只读兼容，新任务使用 v11 重筛并保留版本信息。
