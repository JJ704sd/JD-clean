...
材料表下方的“筛选岗位”和“筛选状态”下拉框会立即过滤当前列表；“全部岗位/全部状态”恢复完整列表。筛选只改变显示，不会删除材料，也不会改变导入时的分流规则。
...
在设置页选择 openai-compatible 或 minimax，填写 HTTPS Base URL（通常以 /v1 结尾）。点击“获取模型列表”会读取兼容接口的 `/models` 并填入可下拉选择的模型；供应商没有该接口时仍可手动填写模型名。密钥存入 Windows 凭据库或 macOS Keychain，普通配置文件不保存密钥。保存配置、连接测试或 AI 分析时输入的 Key 都会持续保存到系统凭据库，并按“协议+Base URL”复用，同一端点切换模型不需要重复填写；重新打开应用只需保持 Key 输入框为空，界面会显示“已安全保存”，不会回显密钥。
