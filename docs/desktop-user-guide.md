...
在设置页选择 openai-compatible 或 minimax，填写 HTTPS Base URL（通常以 /v1 结尾）。点击“获取模型列表”会读取兼容接口的 `/models` 并填入可下拉选择的模型；供应商没有该接口时仍可手动填写模型名。密钥存入 Windows 凭据库或 macOS Keychain，普通配置文件不保存密钥。保存配置、连接测试或 AI 分析时输入的 Key 都会持续保存到系统凭据库；重新打开应用只需保持 Key 输入框为空，界面会显示“已安全保存”，不会回显密钥。
